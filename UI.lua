local library = {
	...
	colors = {
		main = Color3.fromHex('00FAFF'),
		background = Color3.fromHex('101010'),
		outerBorder = Color3.fromHex('0E0E0E'),
		innerBorder = Color3.fromHex('454545'),
		topGradient = Color3.fromHex('000000'),
		bottomGradient = Color3.fromHex('1A1717'),
		sectionBackground = Color3.fromHex('000000'),
		section = Color3.fromHex('00FAFF'),
		otherElementText = Color3.fromHex('00FAFF'),
		elementText = Color3.fromHex('FFFFFF'),
		elementBorder = Color3.fromHex('141414'),
		selectedOption = Color3.fromHex('373737'),
		unselectedOption = Color3.fromHex('282828'),
		hoveredOptionTop = Color3.fromHex('414141'),
		unhoveredOptionTop = Color3.fromHex('323232'),
		hoveredOptionBottom = Color3.fromHex('2D2D2D'),
		unhoveredOptionBottom = Color3.fromHex('232323'),
		tabText = Color3.fromHex('B9B9B9')
	},
	gui_parent = (function()
				...
				if getsynasset then
					...
					elseif #image > 14 and string.sub(image, 1, 14) == "synasseturl://" then
						local x, e = pcall(function()
							...
							local fold = isfolder("./XOP HUB")
							if not fold then
								makefolder("./XOP HUB")
							end
							fold = isfolder("./XOP HUB/Themes")
							if not fold then
								makefolder("./XOP HUB/Themes")
							end
							fold = isfolder("./XOP HUB/Themes/SynapseAssetsCache")
							if not fold then
								makefolder("./XOP HUB Themes/SynapseAssetsCache")
							end
							if not fold or not isfile("./XOP HUB/Themes/SynapseAssetsCache/" .. codename .. ".dat") then
								local res = game:HttpGet(string.sub(image, 15))
								if res ~= nil then
									writefile("./XOP HUB/Themes/SynapseAssetsCache/" .. codename .. ".dat", res)
								end
							end
							return getsynasset(readfile("./XOP HUB/Themes/SynapseAssetsCache/" .. codename .. ".dat"))
						end)
						if x and e ~= nil then
							return e
						end
...
function library:CreateWindow(options, ...)
	...
	if windowName and #windowName > 0 and library.WorkspaceName == "XOP HUB" then
		library.WorkspaceName = convertfilename(windowName, "XOP HUB")
	end
		...
		function tabFunctions:CreateSection(options, ...)
			...
			if isfolder and makefolder and listfiles and readfile and writefile then
				function sectionFunctions:AddPersistence(options, ...)
					...
					local realDropdownHolderList = Instance_new("UIListLayout")
					local dropdownEnabled = false
					if not isfolder("./XOP HUB") then
						makefolder("./XOP HUB")
					end
					local common_string = "./XOP HUB/" .. tostring(custom_workspace or library.WorkspaceName)
					local function resolvelist(nofold)
						if custom_workspace ~= options.Workspace then
							custom_workspace = options.Workspace
							common_string = "./XOP HUB/" .. tostring(custom_workspace or library.WorkspaceName)
						end
	...
	function windowFunctions:CreateDesigner(options, ...)
		...
		local daaata = {{"AddTextbox", "__Designer.Textbox.ImageAssetID", backgroundsection, {
			Name = "Image Asset ID",
			Placeholder = "rbxassetid://9836679987",
			Flag = "__Designer.Background.ImageAssetID",
			Value = "rbxassetid://9836679987",
